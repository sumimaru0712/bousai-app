...
import { BAG_GAME_SECONDS, BAG_ITEMS, type BagItem } from "@/lib/minigames";
...
export default function BagGamePage() {
  const { state, awardPoints, addActivity } = useAppState();
  const [phase, setPhase] = useState<Phase>("intro");
  const [items, setItems] = useState<BagItem[]>(() => shuffle(BAG_ITEMS));
  const [selected, setSelected] = useState<Set<string>>(new Set());
  const [timeLeft, setTimeLeft] = useState(BAG_GAME_SECONDS);
  ...
  function startGame() {
    setItems(shuffle(BAG_ITEMS));
    setSelected(new Set());
    setTimeLeft(BAG_GAME_SECONDS);
    setScore(null);
    setPhase("playing");
  }
  ...
  function finishGame() {
    ...
    for (const item of items) {
      ...
      if (item.correct && isSelected) raw += 1;
      if (!item.correct && isSelected) raw -= 1;
    }
    const finalScore = Math.max(0, Math.min(totalCorrect, raw));
    setScore(finalScore);
    awardPoints(finalScore);
    addActivity(
      `非常持ち出し袋あてゲームで${finalScore}/${totalCorrect}点とったよ！`
    );
    setPhase("result");
  }
      ...
      <div className="flex flex-col gap-6">
        ...
        <section className="rounded-3xl bg-white p-6 text-center shadow-sm ring-1 ring-orange-100">
          ...
          <p className="mt-2 text-sm text-zinc-500">
            {BAG_GAME_SECONDS}秒のあいだに、非常持ち出し袋に入れるべき物をタップしよう
          </p>
          <button
            type="button"
            onClick={startGame}
            ...
            {items.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`flex flex-col items-center gap-1 rounded-2xl border-2 py-4 text-xs font-bold transition-colors ${
                    isSelected
    ...
    <div className="flex flex-col gap-6">
      ...
      <section className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-orange-100">
        ...
        <ul className="mt-3 flex flex-col gap-2">
          {items.map((item) => {
            const isSelected = selected.has(item.id);
            const good = item.correct === isSelected;
            return (
              <li
                key={item.id}
