...
import type { DangerCategory } from "@/lib/types";
...
export default function DangerMarkDetailPage({
  ...
  const { state, toggleMarkResolved } = useAppState();
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="overflow-hidden rounded-3xl bg-white shadow-sm ring-1 ring-orange-100">
        ...
        <div className="p-6">
          ...
          <button
            type="button"
            onClick={() => toggleMarkResolved(photo.id, mark.id)}
            className={`mt-6 w-full rounded-full py-4 text-lg font-extrabold text-white shadow-md transition-colors ${
              mark.resolved
                ? "bg-zinc-400 hover:bg-zinc-500"
                : "bg-green-600 hover:bg-green-700"
            }`}
          >
            {mark.resolved ? "✓ 対策ずみ（もどす）" : "対策できた！"}
          </button>
        </div>
      </div>
    </div>
